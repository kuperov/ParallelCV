...
class TestLogWelford(unittest.TestCase):
    ...
    def testBatchMoments(self):
        ...
        w, _ = jax.lax.scan(lambda carry_w, lx: (batch_log_welford_add(lx, carry_w), None), w0, lxs)
        self.assertAlmostEqual(batch_log_welford_mean(w), jnp.log(jnp.mean(xs_bm)), places=2)
        self.assertAlmostEqual(batch_log_welford_var(w), jnp.log(jnp.var(xs_bm)), places=2)
        self.assertAlmostEqual(batch_log_welford_var(w, ddof=0), jnp.log(jnp.var(xs_bm, ddof=0)), places=2)
        self.assertAlmostEqual(batch_log_welford_var(w, ddof=1), jnp.log(jnp.var(xs_bm, ddof=1)), places=2)
...
class TestMultivariateWelford(unittest.TestCase):
    ...
    def testBatchVectorMoments(self):
        ...
        w, _ = jax.lax.scan(lambda w_carry, x: (batch_vector_welford_add(x, w_carry), None), w0, data)
        bmeans = xbs.mean(axis=1)
