...
def _init_model(data_key, N=100, p=p):
    ...
    y = X @ beta0 + tfd.Normal(loc=0, scale=jnp.sqrt(sigsq0)).sample(
        sample_shape=(N,), seed=y_key
    )
    ...
    def log_pred(theta, fold_id):
        ...
        return -0.5 * (
            npred * jnp.log(2 * jnp.pi) + npred * jnp.log(sigsq) + esq.sum() / sigsq
        )
    ...
    def make_initial_pos(key):
        ...
        theta = Theta(
            beta=jax.random.normal(key=k1, shape=(p,)), sigsq=jax.random.normal(key=k2)
        )
        return theta
