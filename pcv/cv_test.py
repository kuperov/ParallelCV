...
class TestCVSchemes(unittest.TestCase):
    def test_loo(self):
        ...
        assert_trees_all_close(loo.test_mask(1), tstm)
        assert_trees_all_close(loo.train_mask(1), jnp.logical_not(tstm))
        Stest1 = loo.S_test(1)
        I = jnp.eye(120)
        assert_same_structure(Stest1, I[tstm, :])
        assert_trees_all_close(Stest1, I[tstm, :])
        Strain1 = loo.S_train(1)
        assert_same_structure(Strain1, I[jnp.logical_not(tstm), :])
        assert_trees_all_close(Strain1, I[jnp.logical_not(tstm), :])
    ...
    def test_kfold(self):
        ...
        assert_trees_all_close(fivefold.train_mask(0), tstm)
        assert_trees_all_close(fivefold.test_mask(0), jnp.logical_not(tstm))
    ...
    def test_hvblock(self):
        ...
        assert_trees_all_close(hvblock.test_mask(19), tstm)
        assert_trees_all_close(hvblock.train_mask(19), tram)
